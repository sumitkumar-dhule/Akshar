package com.electrodiligent.core.util

import androidx.compose.ui.unit.Dp
import androidx.compose.ui.unit.dp

object Dimension {
    val SIZE_MEDIUM: Dp = 50.dp
    val SIZE_LARGE: Dp = 100.dp
    val SIZE_HUGE: Dp = 200.dp
    val PADDING_SMALL: Dp = 8.dp
    val PADDING_MEDIUM: Dp = 16.dp
    val PADDING_LARGE: Dp = 24.dp
    val PADDING_TITLE: Dp = 0.dp
    val PADDING_BOTTOM: Dp = 1.dp
    val PADDING_BANNER_AD: Dp = 50.dp
    val PADDING_LARGE_BANNER_AD: Dp = 100.dp
    val PADDING_TOOLBAR: Dp = 80.dp
}